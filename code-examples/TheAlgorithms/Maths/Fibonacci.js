...
function FibonacciIterative(nth) {
  const sequence = []
...
function FibonacciRecursiveDP(stairs) {
...
function FibonacciDpWithoutRecursion(number) {
  const table = []
  table.push(1)
  table.push(1)
...
function copyMatrix(A) {return A.map(row => row.map(cell => cell))}
...
function FibonacciMatrixExpo(n) {
  // F(0) = 0, F(1) = 1
  // F(n) = F(n-1) + F(n-2)
